...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            self.epsilon = math.pow(self.alpha,self.trialcount)
            #self.epsilon = math.exp(-self.alpha * self.trialcount) 
            #self.epsilon = 1/math.pow(self.trialcount,2)
    ...
    def get_maxQ(self, state):
        ...
        MAX = -1000 # ensures I account for negative Q values in the first element so I don't miss it

        ### Is there an easier way to do this?? --- to my reviewer :)
        #tielist = list()
        for key, value in self.Q[state].iteritems():
            if MAX <= value:
                MAX = value
        d = {k: v for k, v in self.Q[state].items() if v == MAX} # Verify how many ties we have
        maxQ = random.choice(d.keys())
        #raw_input("multiple ties. Press Enter to continue...",d.keys())


        """        tielist.append(key)
        if len(tielist)>1:
            maxQ=random.choice(tielist)
            #print("multiple ties")
            #raw_input("multiple ties. Press Enter to continue...")
        else:
            maxQ = tielist[0]"""
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning == True:
            ...
            if currentQ != 0:
                print "Learning Change = {:.2f}%".format((currentQ-self.Q[state][action])/currentQ * 100) 
        return
...
def run():
    ...
    agent = env.create_agent(LearningAgent,learning=True,alpha=0.99)#.99
